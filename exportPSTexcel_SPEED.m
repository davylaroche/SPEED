        ...
        for c2 = 1:size(listesessions,1) % Boucle par session
            ...
            if strcmpi(listesessions(c2).name,'M0') || strcmpi(listesessions(c2).name,'M6')
                ...
                try
                    ...
                    pstangles = fichierpst_angles.pst_angles; % Structure avec les deux côtés
                    listecotes = fieldnames(pstangles); % récupération de la liste des 2 côtés
                    
                    for c3 = 1:size(listecotes,1) % boucle par côté
                        clear nomcote nomcote_t listeessais
                        
                        
                        nomcote = listecotes(c3); % Définition du nom du côté
                        nomcote_t = char(nomcote); % conversion du nom en charactère
                        
                        listeessais = fieldnames(pstangles.(nomcote_t)); % Récupération de la liste des essais
                        ...
                        for c4 =1:size(listeessais,1) % Boucle par essai
                            ...
                            nombredelignes = [];
                            nomessai = listeessais(c4); % Définition du nom de l'essai
                            nomessai_t = char(nomessai); % conversion du nom en charactère
                            
                            listevariables = fieldnames(pstangles.(nomcote_t).(nomessai_t)); % Définiton de la liste des variables
                            
                            counter3 = 6; % Démarrage 6ème colonne
                            ...
                            for c5=1:size(listevariables,1) % Boucle pour chaque variable
                                ...
                                nomvariable = listevariables(c5); % Définition du nom de la variable
                                nomvariable_t = char(nomvariable); % Conversion du nom en charactère
                                ...
                                valeurs = pstangles.(nomcote_t).(nomessai_t).(nomvariable_t); % Récupération de la valeur dans la matrice
                                counter2 = counter;
                                
                                try % Essai de remplissage des valeurs dans le tableau en fonction du nom de la variable
                                    
                                    if strcmpi(nomvariable_t,'Footstrike_a') % Création de 2 footstrike par cycle
                            ...
                            %% Après le remplissage des l'ensemble des valeurs remplissage des lignes avec essais, cycle, côté
                            nombremaxdelignes = max(nombredelignes(:));
                            
                            counter4 = counter;
                            for c7 = 1:nombremaxdelignes % boucle pour écrire les cycles
                            ...
                            if isempty(nombremaxdelignes)
                                ...
                                counter = counter + nombremaxdelignes;%incrémentation du compteur
                            end
                    ...
                    warning('Le fichier de la session ne peut être chargé pour le volontaire');
                    volontaire
                    session
